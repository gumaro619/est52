...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <h5 class="card-title">Sobre la credencialización</h5>
            <p class="card-text">La credencial es  el instrumento que identifica a un alumno como estudiante  inscrito en  esta institución, El estudiante o su tutor debe solicitarla</p>
            <a href="#" class="btn btn-primary">!Entendido¡</a>
        </div>
    </div>
    
    <p>Seleccione al alumno para generar su credencial virtual </p>
        ...
        <tbody>
            @forelse($alumnos as $alumno)
                <tr>
                    ...
                    <td>{{ ($alumno->tutor->persona->nombre ?? 'No')." ".($alumno->tutor->persona->apellido_p ?? 'Aasignado')}}</td>
                    <td>{{ $alumno->grupo->nombre ?? 'No asignado'}}</td>
                    <td>
                        <a class="btn btn-info" href="{{ route('credencializacion.show',$alumno->id) }}"  target="_blank">Consultar</a>
                        <a class="btn btn-success" href="{{ route('credencializacion.edit',$alumno->id) }}"  target="_blank">Descargar</a>
                    </td>
